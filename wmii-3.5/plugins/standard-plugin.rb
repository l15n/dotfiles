...
# 
# This file must be placed in $HOME/.wmii-3.5/plugins.
# 
...
# * copy the relevant code to another file under $HOME/.wmii-3.5/plugins
...
Plugin.define "standard"  do
  ...
  bar_applet("volume", 990) do |wmii, bar|
    ...
    Thread.new{ loop { update_volume[0]; sleep 3 } }
  ...
  bar_applet("mode", 980) do |wmii, bar|
    ...
    handlers = nil # Will hold the on_key KeyHandlers
    toggle = proc do
      if raw_mode
        wmii.write("/keys", saved_keys)
        raw_mode = false
        bar.data = "-N-"
        LOGGER.info "Entering NORMAL mode."
      else
        saved_keys = wmii.read("/keys")
        wmii.write("/keys", handlers.map { |h| h.key }.join("\n"))
        raw_mode = true
        bar.data = "-R-"
        LOGGER.info "Entering RAW mode."
      end
    end
    handlers = Array(wmii.on_key(*keys, &toggle))
    bar.on_click(&toggle)
    bar.data = "-N-"
  end
...
# Originally by Wael Nasreddine <wael@phoenixlinux.org>.
  ...
  bar_applet("mpd", 100) do |wmii, bar|
    ...
    bar.on_click(MOUSE_BUTTON_RIGHT) do
      ...
      wmii.write("/tag/sel/ctl", "select ~")
      term = wmii.plugin_config["standard"]["x-terminal-emulator"] || "xterm"
      system "ssid #{term} -e ncmpc &"
    end
  end
...
# # {{{ CPU info
  bar_applet("cpuinfo", 800) do |wmii, bar|
    Thread.new do
      loop do
        cpuinfo = IO.readlines("/proc/cpuinfo").grep(/cpu MHz/).first.split[-1].sub(/\..*$/,'')
...
# {{{ Status bar
  bar_applet("status", 0, "STATUS BAR --- init") do |wmii, bar|
    ...
    left_action = wmii.plugin_config["standard:status"]["left_click_action"] || 
                  lambda { fl[]; system "tzwatch | ssid #{xmessagebox} &"; toggle_fl[] }
    right_action = wmii.plugin_config["standard:status"]["right_click_action"] || 
                   lambda { fl[]; system "ncal -y | ssid #{xmessagebox} &"; toggle_fl[] }
    middle_action = wmii.plugin_config["standard:status"]["middle_click_action"] || 
                   lambda {  fl[]; system "ssid #{term} -e top &"; toggle_fl[] }
...
# {{{ actions (internal and WMIIRC_HOME/*) (w/ history)
  standard_internal_actions = {
    "browser" => lambda do |wmii, *selection|
      ...
      selection ||= `sselp`.strip
      case browser = ENV["BROWSER"]
      when nil: system "ssid /etc/alternatives/x-www-browser '#{selection}' &"
      else system "ssid #{browser} '#{selection}' &"
      end
    end,
  ...
  binding("execute-action", "MODKEY-a") do |wmii,|
    ...
    result = wmii.wmiimenu(list) do |choice|
      ...
      if internal_actions.has_key? cmd
        ...
        system("ssid #{WMIIRC_HOME}/#{choice} &") if /^\s*$/ !~ choice
      end
    end
...
# {{{ Run program with given tag
  ...
  feature("tag-bar") do |wmii|
    wmii.register("CreateTag") do |tag|
      wmii.create "/lbar/#{tag}"
      wmii.write  "/lbar/#{tag}", "#{wmii.normcolors} #{tag}"
    end
    wmii.register("DestroyTag")   { |tag| wmii.remove "/lbar/#{tag}" }
    wmii.register("FocusTag")     { |tag| wmii.write "/lbar/#{tag}", "#{wmii.focuscolors} #{tag}" }
    wmii.register("UnfocusTag")   { |tag| wmii.write "/lbar/#{tag}", "#{wmii.normcolors} #{tag}" }
    wmii.register("UrgentTag")    { |type, tag| wmii.write "/lbar/#{tag}", "*#{tag}" }
    wmii.register("NotUrgentTag") { |type, tag| wmii.write "/lbar/#{tag}", tag }
    wmii.register("LeftBarClick") { |n, tag| wmii.write "/ctl", "view #{tag}" }
  end
...
# {{{ Remove the current tag from the current window,
#     deleting it if it has no tags.
  binding("detag", "MODKEY-Shift-d") do |wmii,|
    if wmii.curr_client_tags.size == 1
      wmii.write "/client/sel/ctl", "kill"
    else
      wmii.retag_curr_client "-#{wmii.curr_view}"
    end
  end
...
#  bookmark        take current X11 primary selection (with sselp), ask
  ...
  def refine_selection(expression, choices=self.bookmarks)
    ...
    choices.select do |bm|
      criteria.all? do |criterion|
        ...
        when /:.+$/     : bm.tags.include?(criterion)
...
BOOKMARK_FILE = File.join(ENV["HOME"], ".wmii-3.5", "bookmarks.txt")
...
Plugin.define "standard"  do
  ...
  feature("bookmark:actions") do |wmii|
...
=end
    ...
    hash.update(
    {
      ...
      "bookmark-add" => lambda do |wmii|
        ...
        options = [`xclip -o -selection clipboard`, `sselp`].reject {|x| x.strip.empty?}
